...
async def update_principals(
    ...
    for action in principals.fields:
        ...
        if action == "add":
            user.principals = list(set(user.principals + principals.add))
        elif action == "delete":
...
async def update_passwd(
    password: UpdatePasswd, current_user: User = Depends(get_current_user)
):
    return password
...
async def update_user(
    data: UserData, current_user: User = Depends(get_current_user)
):
    return data
...
async def signup(new_user: UserSignup):
    if new_user.password != new_user.confirm_password:
        raise HTTPException(
            status_code=412, detail="Invalid confirmation password!"
        )
    new_user = UserInDB(
        **{
            **new_user.dict(),
